...
class CartController {
    ...
    const startDate = moment(req.body.startDate, 'YYYY-MM-DD').toDate();
    const endDate = moment(req.body.endDate, 'YYYY-MM-DD').toDate();

    const timeDifference = Math.abs(endDate - startDate);
    const dateRental = Math.ceil(timeDifference / (1000 * 60 * 60 * 24));
    ...
    vnp_Params['vnp_Amount'] = amount * dateRental * 100;
    ...
    // add rentalisting
    const customerInfor = new RentalListing(req.body);
    customerInfor.save();
    res.redirect(vnpUrl)
